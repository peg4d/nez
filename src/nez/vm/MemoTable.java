...
//	public final static Node NonTransition = new ParsingObject();
...
public abstract class MemoTable {
	...
	protected void setMemo(long pos, int memoPoint, Node result, int consumed) {
		MemoEntry m = newMemo();
	...
	protected void setMemo2(long pos, int memoPoint, int stateValue, Node result, int consumed) {
		MemoEntry m = newMemo();
		m.memoPoint = memoPoint;
...
class SlidingWindowTable extends MemoTable {
	...
	@Override
	protected final void setMemo(long pos, int memoPoint, Node result, int consumed) {
		int index = (int)(pos % size);
		MemoEntry m = this.memoArray[index][memoPoint];
		m.key = pos;
		m.result = result;
...
class ElasticTable extends MemoTable {
	...
	@Override
	protected final void setMemo2(long pos, int memoPoint, int stateValue, Node result, int consumed) {
		long key = longkey(pos, memoPoint, shift);
		int hash =  (int)(key % memoArray.length);
		MemoEntry m = this.memoArray[hash];
		m.key = key;
